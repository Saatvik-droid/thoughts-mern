...
const ThoughtsForm = ({ type, id }) => {
  const thought = useSelector((state) =>
    state.thoughtsState.thoughts.filter((thought) => thought._id === id)
  );
  const user = useSelector((state) => state.authState.authData);

  const [thoughtData, setThoughtData] = useState(
    thought[0] ? thought[0] : { title: "", body: "" }
  );
      ...
      <form autoComplete="off" onSubmit={handleSubmit}>
        <Grid container spacing={2}>
          <CustomtextField
            ...
            defaultValue={thoughtData.title}
